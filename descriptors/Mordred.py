...
#generate Mordred descriptors for 16 rings
mols = [Chem.MolFromSmiles(smi) for smi in ['c1cc(c(cc1)C)O',
                                            'c1c(c(c(cc1)C)O)C',
...
df = calc.pandas(mols)
df.to_csv("mordred_rings.csv")

#generate Mordred descriptors for 36 linkers
mols = [Chem.MolFromSmiles(smi) for smi in ['C(C/N=C/C)/N=C/C',
                                            'C(CC/N=C/C)/N=C/C',
