...
#generate EI descriptors for 16 rings
mols = [Fingerprinter.FingerprintMol(Chem.MolFromSmiles(smi))[1] for smi in    ['c1cc(c(cc1)C)O',
                                                                                'c1c(c(c(cc1)C)O)C',
...
data = pd.DataFrame(mols)

data.to_csv("EI_rings.csv")




#generate EI descriptors for 36 linkers
mols2 = [Fingerprinter.FingerprintMol(Chem.MolFromSmiles(smi))[1] for smi in   ['C(C/N=C/C)/N=C/C',
                                                                                'C(CC/N=C/C)/N=C/C',
...
data = pd.DataFrame(mols2)

data.to_csv("EI_linkers.csv")
