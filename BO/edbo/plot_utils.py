...
def plot_convergence(data, batch_size, avg=False, export_path=None):
    ...
    ax1.plot(index, max_obs, 'o-', color="coral")
    ax2.plot(index[:-1], conv_rate, '-o', color='sienna')
    ax2.set(xlabel='Batch', ylabel='d(Max Observed)/d(Batch)')
    ax2.set_title('Rate')
    fig.tight_layout()
...
def plot_avg_convergence(data, batch_size, export_path=None):
    ...
    ax1.plot(index, mean, 'o-', color="coral")
    ax1.fill_between(index, mean-std, mean+std, color='mediumaquamarine', alpha=0.3)
    ax2.plot(index[:-1], conv_rate, '-o', color='sienna')
    ax2.set(xlabel='Batch', ylabel='d(Max Observed)/d(Batch)')
